...
class delete_botdataentity extends \external_api {
    ...
    public static function execute($realid) {
        ...
        $result['result'] = 'Error deleting the element';
        $result['realid'] = '';

        if ($record->id !== null){
            $DB->delete_records('local_botmanager_dataentity', ['id' => $record->id]);
            $result['result'] = 'Element successfully deleted from the database';
            $result['realid'] = $record->realid;
        }
    ...
    public static function execute_returns(): external_single_structure {
        return new external_single_structure([
                'result' => new external_value(PARAM_TEXT, 'operation result'),
